  ...
  case topExpr of
    ...
    Case e matches ->
      let f :: Alt -> Core.Alt
       ...
       in Core.Case (desugar e) (fmap f matches)
    IntLiteral n ->
      Core.PrimInt n
    TextLiteral t ->
      desugarText t
...
desugarLet decls body =
  ...
  where
    ...
    addLet recursiveBindings acc =
      let f :: (Id, (Expr, Type)) -> (Id, (Core.Expr, Type))
          f (id, (expr, typ)) =
            (id, (desugarBinding id expr, typ))
       in Core.Let (HashMap.fromList (fmap f recursiveBindings)) acc
