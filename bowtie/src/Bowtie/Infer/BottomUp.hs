...
bottomUpLet env ms bindings expr = do
  ...
  where
    ...
    newC aa =
      let g :: (Id, (Expr, Type)) -> Constraints -> Constraints
       ...
       in foldr g mempty (OrderedMap.toList bindings)
...
bottomUpCase env ms targetExpr alts = do
  ...
  let inferAlt :: (TypeScheme, [(Id, Type)], Expr) -> m (TypeScheme, Assumptions, Constraints, Type)
      inferAlt (scheme, bindings, expr) = do
        let bindingIds = fmap fst bindings
        (aa, cc, t) <- bottomUp env (ms <> Set.fromList bindingIds) expr
        let addC :: (Id, Type) -> Constraints -> Constraints
    ...
    addConInfo (Alt conId bindings expr) =
      case lookup conId env of
        ...
        Just (TypeScheme polyVars typ) -> do
          let g :: (Type, [(Id, Type)]) -> Id -> m (Type, [(Id, Type)])
              g (t, xs) binding =
                case t of
                  TArrow targ t2 ->
